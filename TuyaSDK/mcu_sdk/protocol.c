...
const DOWNLOAD_CMD_S download_cmd[] =
{
  ...
  {DPID_PAUSE, DP_TYPE_BOOL},
  {DPID_MODE, DP_TYPE_ENUM},
  {DPID_COUNT_REALTIME, DP_TYPE_VALUE},
  {DPID_TIME_REALTIME, DP_TYPE_VALUE},
  {DPID_CALORIES_REALTIME, DP_TYPE_VALUE},
  {DPID_SPEED_REALTIME, DP_TYPE_VALUE},
  {DPID_COUNT_TOTAL, DP_TYPE_VALUE},
  {DPID_TIME_TOTAL, DP_TYPE_VALUE},
  {DPID_CALORIES_TOTAL, DP_TYPE_VALUE},
  {DPID_TARGET_TIME, DP_TYPE_VALUE},
  {DPID_TARGET_COUNT, DP_TYPE_VALUE},
  {DPID_BATTERY_STATE, DP_TYPE_ENUM},
  {DPID_WEIGHT, DP_TYPE_VALUE},
  {DPID_GENDER, DP_TYPE_ENUM},
  {DPID_HEIGHT, DP_TYPE_VALUE},
};
...
void all_data_update(void)
{
  ...
  // This code is automatically generated for the platform. Please modify each function according to the actual data
    mcu_dp_bool_update(DPID_START,当前开始); //BOOL型数据上报;
    mcu_dp_bool_update(DPID_PAUSE,当前暂停); //BOOL型数据上报;
    mcu_dp_enum_update(DPID_MODE,当前跳绳模式); //枚举型数据上报;
    mcu_dp_value_update(DPID_COUNT_REALTIME,当前实时计数); //VALUE型数据上报;
    mcu_dp_value_update(DPID_TIME_REALTIME,当前实时计时); //VALUE型数据上报;
    mcu_dp_value_update(DPID_CALORIES_REALTIME,当前实时卡路里); //VALUE型数据上报;
    mcu_dp_value_update(DPID_SPEED_REALTIME,当前实时跳绳速度); //VALUE型数据上报;
    mcu_dp_value_update(DPID_COUNT_TOTAL,当前总跳绳个数); //VALUE型数据上报;
    mcu_dp_value_update(DPID_TIME_TOTAL,当前总时长); //VALUE型数据上报;
    mcu_dp_value_update(DPID_CALORIES_TOTAL,当前总卡路里); //VALUE型数据上报;
    mcu_dp_value_update(DPID_TARGET_TIME,当前目标时间); //VALUE型数据上报;
    mcu_dp_value_update(DPID_TARGET_COUNT,当前目标个数); //VALUE型数据上报;
    mcu_dp_enum_update(DPID_BATTERY_STATE,当前电池电量); //枚举型数据上报;
    mcu_dp_value_update(DPID_WEIGHT,当前体重); //VALUE型数据上报;
    mcu_dp_enum_update(DPID_GENDER,当前性别); //枚举型数据上报;
    mcu_dp_value_update(DPID_HEIGHT,当前身高); //VALUE型数据上报;
...
/*****************************************************************************
函数名称 : dp_download_pause_handle
功能描述 : 针对DPID_PAUSE的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_pause_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为BOOL
    unsigned char ret;
    //0:off/1:on
    ...
    pause = mcu_get_dp_download_bool(value,length);
    if(pause == 0) {
        //bool off
    }else {
        //bool on
    ...
    //There should be a report after processing the DP
    ret = mcu_dp_bool_update(DPID_PAUSE,pause);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_target_time_handle
功能描述 : 针对DPID_TARGET_TIME的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_target_time_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为VALUE
    unsigned char ret;
    unsigned long target_time;
    
    target_time = mcu_get_dp_download_value(value,length);
    /*
    ...
    */
    ...
    //There should be a report after processing the DP
    ret = mcu_dp_value_update(DPID_TARGET_TIME,target_time);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_target_count_handle
功能描述 : 针对DPID_TARGET_COUNT的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_target_count_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为VALUE
    unsigned char ret;
    unsigned long target_count;
    
    target_count = mcu_get_dp_download_value(value,length);
    /*
    //VALUE type data processing
    ...
    */
    ...
    //There should be a report after processing the DP
    ret = mcu_dp_value_update(DPID_TARGET_COUNT,target_count);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_weight_handle
功能描述 : 针对DPID_WEIGHT的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_weight_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为VALUE
    unsigned char ret;
    unsigned long weight;
    
    weight = mcu_get_dp_download_value(value,length);
    /*
    //VALUE type data processing
    ...
    */
    ...
    //There should be a report after processing the DP
    ret = mcu_dp_value_update(DPID_WEIGHT,weight);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_gender_handle
功能描述 : 针对DPID_GENDER的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_gender_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为ENUM
    unsigned char ret;
    unsigned char gender;
    
    gender = mcu_get_dp_download_enum(value,length);
    switch(gender) {
        case 0:
        break;
    ...
    //There should be a report after processing the DP
    ret = mcu_dp_enum_update(DPID_GENDER, gender);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_height_handle
功能描述 : 针对DPID_HEIGHT的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_height_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为VALUE
    unsigned char ret;
    unsigned long height;
    
    height = mcu_get_dp_download_value(value,length);
    /*
    //VALUE type data processing
    ...
    */
    ...
    //There should be a report after processing the DP
    ret = mcu_dp_value_update(DPID_HEIGHT,height);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
unsigned char dp_download_handle(unsigned char dpid,const unsigned char value[], unsigned short length)
{
  ...
  ***********************************/
  ...
  switch(dpid)
  {
        ...
        break;
        case DPID_TARGET_TIME:
            //目标时间处理函数
            ret = dp_download_target_time_handle(value,length);
        break;
        case DPID_TARGET_COUNT:
            //目标个数处理函数
            ret = dp_download_target_count_handle(value,length);
        break;
        case DPID_WEIGHT:
            //体重处理函数
            ret = dp_download_weight_handle(value,length);
        break;
        case DPID_GENDER:
            //性别处理函数
            ret = dp_download_gender_handle(value,length);
        break;
        case DPID_HEIGHT:
            //身高处理函数
            ret = dp_download_height_handle(value,length);
        break;
