...
static unsigned char bt_uart_rx_buf_temp[3] = {0};
static unsigned short uart_data_len =  0;
static volatile unsigned short UART_RX_Count = 0;


static unsigned char mcu_common_uart_data_unpack(unsigned char rec_data)
{
    unsigned char ret = FALSE;

    bt_uart_rx_buf_temp[0] = bt_uart_rx_buf_temp[1];
    bt_uart_rx_buf_temp[1] = bt_uart_rx_buf_temp[2];
    bt_uart_rx_buf_temp[2] = rec_data;

    if((bt_uart_rx_buf_temp[0]==0x55||bt_uart_rx_buf_temp[0]==0x66)&&(bt_uart_rx_buf_temp[1]==0xAA)&&(bt_uart_rx_buf_temp[2]==0x00))
    {
    ...
    case MCU_UART_REV_STATE_FOUND_HEAD:
        bt_uart_rx_buf[UART_RX_Count++] = rec_data;
    ...
    case MCU_UART_REV_STATE_FOUND_LEN_L:
        bt_uart_rx_buf[UART_RX_Count++] = rec_data;   //DATA
        uart_data_len--;
        if(uart_data_len==0)
        {
            current_uart_rev_state_type = MCU_UART_REV_STATE_FOUND_DATA;
    ...
    case MCU_UART_REV_STATE_FOUND_DATA:
        bt_uart_rx_buf[UART_RX_Count++] = rec_data;  //sum data
        ret = TRUE;
        break;
    default:
        my_memset(bt_uart_rx_buf_temp,0,3);
